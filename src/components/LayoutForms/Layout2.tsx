...
const Layout2 = ({ title, subtitle, ref_int, _id, isEditing }: IItem) => {
    ...
    if (isEditing) {
      const callLayoutApi = async () => {
        ...
        if (check !== -1) {
          ...
          setStock(infos.posters_layouts[check].stock);
        }
      };
      callLayoutApi();
    }
  ...
  return (
    <fieldset>
      <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4">
        <img src={imagesUri[1]} className="max-h-[500px]" />
        <div className="my-3 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2 mt-3 flex-row-reverse">
            ...
            <a
              target="_blank"
              href={`/admin/layout/2/${title}&${subtitle}&${valuePreviously}&${value}&${ref_int}&${unidadeVenda}&${stock}&true&true&${new Date().toDateString()}&${new Date().toDateString()}`}
